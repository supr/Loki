  ...
  def compare(o1: JValue, o2: JValue): Int = {
    o1 match {
      ...
      case f:JDouble => o2 match {
        case JNothing|JNull|JBool => 1
        case i:JInt => BigInt(f.values.toInt).compare(i)
        case f2:JDouble => f.compare(f2)
        case _ => -1
      }

      case s:JString => o2 match {
        case JNothing|JNull|JBool|JInt|JDouble => 1
        case s2:JString => collator.compare(s.values, s2.values)
        case _ => -1
      }

      case a:JArray => o2 match {
        case JNothing|JNull|JBool|JInt|JDouble|JString => 1
        case a2:JArray => {
      ...
      case o:JObject => o2 match {
        case JNothing|JNull|JBool|JInt|JDouble|JString|JArray => 1
        case o2:JObject => {
          val r = o.obj.zip(o2.obj).foldLeft(0)((res, e) => if (res == 0) collator.compare(e._1.name, e._2.name) else result)
